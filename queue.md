...
and annotated slice is `docs/slices.md` - S1-S17, S29-S32 and S34-S36 today.
...
| **S23** | **Belfry discretion-quality criterion.** Turn S8b's trace distinction into a narrow, gradable quality question without general-referee claims. It was blocked behind S29 - a quality metric read over a 60% fallback rate measures the fallback. **UNBLOCKED 2026-09-01:** S29 closed on the finding that no arm will carry `recovered > 0`, so the entry condition it was waiting on is unbuyable and this row now asks for what its own reasoning meant - a record under the void bar with every scored call the model's. The v2 pair meets it at 0/20 fallback over 20 paired legal traces. | judgment | codex | a belfry adjudicator record under the 10% void bar, every scored call the model's - **met by the v2 pair** | pre-committed criterion, control, scorer and frozen recipe |
