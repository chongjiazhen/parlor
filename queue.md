...
| **changeling** | done | yes - gate #3 HOLDS on BOTH decks (S5 five-seat, S19 six-seat waker) | the `waker` deck is seated and read, so the deck debt is PAID. What is left is the question that deck was built to ask and did not: **does the waker SEAT vote better than the table?** It is not in the blind stratum the gate counts, so it needs its own read off the same records - no GPU. `kindred` deck B is still paper and needs `require_seated_kin` |
