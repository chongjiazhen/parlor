...
|---|---|---|---|---|---|
| **S19** | **Waker deck campaign.** Run S18's frozen arms and render the verdict. **LAUNCH-READY 2026-09-02** - `eval/runs/changeling-waker.cmd waker1 200 12000 qwen36-35b-a3b-iq3`, ~6.1 h, and it runs its own random control after. Settings bind from `docs/changeling-waker-criterion.md`, NOT from the row or the launcher. Arm the model first: `kill-gpu`, then arm, then read `/v1/models`. **The verdict must disclose one interim peek:** at game 50 the blind stratum was inspected to check the criterion's power assumption (1.354 blind votes/game against the priced 1.383 - it holds), and the gate statistic was printed in the same pass. Nothing was acted on and the arm ran its full 200, so the fixed-N analysis stands - but the criterion spends no alpha, so the interim figure has no standing and is not citable. Say so in the writeup rather than letting it go unrecorded. **The verdict needs a NEW module, `eval/waker_verdict.py`; do not retrofit `eval.s5_verdict`.** That tool takes a record path and its arithmetic is now table-general (`40f7b9d`), but its BAR and its whole narrative are S2's: `CRITERION_BAR = 0.3595`, "S2 ran no random side", "predicted ~260 blind votes", "clears from a true rate of 42% upward". Pointed at waker1 it would call a six-seat gate against the five-seat bar and print S2's promises over it. It also reproduces a PUBLISHED number, so it is the wrong thing to rebuild under time pressure. Belfry's pattern is one verdict module per criterion (`belfry_live1`/`adjudicator`/`steering`); follow it, reuse `s5_verdict`'s `load`, `seats_in`, `derived_chance` and `blind_chance`, and give it `--transcript` as quorum and belfry now have. | judgment | codex | S18 criterion and recipe committed - **met** | records, transcript, verdict, both fallback rates, and the peek disclosed |
