...
| **quorum** | done | **never** | the live4 arm, and it is pure GPU. Recipe is tracked: `eval/runs/quorum-live4.cmd`, argument-free, asserted field by field against `docs/quorum-live4-criterion.md` §The arm. Score with `eval/quorum_live1_verdict.py`, which binds live4's path and document as one object since 2026-09-01. live1-3 superseded in writing, unrun. Seeds 11200..11219; 5200..5599 / 7000..7399 spent. **`--no-thinking` or the arm voids** - 12.90% fallback without it, 0.00% with |
| **belfry** | done, scoring lane, control instrument, sampled-player arm, S8 referee read, and the live2 arm READ | **never** | S8b is DISTINGUISHABLE and live2's Clause A INFORMS at 20.34% [13.77%, 27.01%] over 1.28% fallback - both in `docs/measurements.md`, read that before citing either. Clause B spans chance and no second arm chases it. What is left is S29's adjudicator retry, which re-baselines S8b |
...
| **S34** | **Referee transcript handle.** DONE in `slice/s34-transcript-handle` (slotB, `0d4cd08`) - held out of `main` by the quorum-live4 freeze. | mechanical | opencode | none | **met**: context manager, mutation-checked against the WinError 32 mask, 131 -> 133 collected |
...
| **S30** | **`cloud_strata` random rows.** DONE in `slice/s30-cloud-strata-random` (slotA, `0eda497`) - held out of `main` by the quorum-live4 freeze. | constrained | opencode | none | **met**: non-model rows in their own cell, never pooled into UNATTRIBUTED, the ValueError still raised for a row that is neither. Cell named NON-MODEL, not RANDOM - `HeuristicPolicy` and `SolverPolicy` set no `last_upstream` either, so a RANDOM label would report the solver arm (live since `3e92509`) as random play |
...
| **S23** | **Belfry discretion-quality criterion.** Turn S8b's trace distinction into a narrow, gradable quality question without general-referee claims. Blocked behind S29 - a quality metric read over a 60% fallback rate measures the fallback. | judgment | codex | S29 landed | pre-committed criterion, control, scorer and frozen recipe |
