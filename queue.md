...
| `slice/notebook-reconcile` | `--notebook`, RESOLVED 2026-09-05 - merge THIS, not `slice/changeling-notebook`, which it contains. The conflict was a design call: the arm names its `note` field by substituting `scratch`, and every ask written after it froze hardcoded the think field instead (`phrasing.ask_vote` in both tables, the `random-active` DISCUSS ask), so a naive merge gives a seat a notebook and nowhere to write in half its turns. `ask_vote` now takes a `{scratch}` placeholder. **The two frozen criteria never actually collided** - the phrasing pin is a sha256 over RENDERED prompts with the notebook off, and it holds. Byte-identity across the merge is checked, not argued: both phrasings x both turn modes x four seeds render to `dd93cb30` on branch and main alike. **Merge only when no changeling arm is live** - UNSAFE against `eval.run_changeling`, four files in its closure |
