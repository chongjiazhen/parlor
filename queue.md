...
| **belfry** | done, scoring lane, control instrument, sampled-player arm, and S8 referee read | **never** | S8b is DISTINGUISHABLE: bounded setup-choice traces differ from seeded random; `docs/measurements.md` holds limits. **live1 is OFF-CRITERION and owes a re-run** - the recorded arm is 100 games at temperature 0.8 with `--no-thinking`; `docs/belfry-live1-criterion.md` promised 60 at 0.0 without it, so `eval.belfry_live1_verdict` exits 3 and every live1 figure is descriptive only |
