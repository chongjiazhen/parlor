...
| `slice/changeling-notebook` | `--notebook`. **Conflicts, tried 2026-09-05 and aborted:** six files, and one is a design call, not a textual fix. The arm injects a `note` field into the ask (`scratch`), and every ask string added since it froze ignores that variable - `phrasing.ask_vote` hardcodes the `think` field with no `{scratch}` placeholder, so a naive resolution drops the note field from the VOTE ask and voids the arm's own stated setting; and the `random-active` DISCUSS ask hardcodes it too, so `--notebook --turns random-active` would drop it there. Giving `ask_vote` a placeholder collides with the phrasing arm's golden hashes over the SAME string, so the two frozen criteria have to be reconciled before either merges. Same class as the `briefing_text` fixed-turns defect, fixed 2026-09-05 |
