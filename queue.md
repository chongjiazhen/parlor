...
and annotated slice is `docs/slices.md` - S1-S15, S32, S35 and S36 today.
...
| **S23** | **Belfry discretion-quality criterion.** Turn S8b's trace distinction into a narrow, gradable quality question without general-referee claims. Blocked behind S29 - a quality metric read over a 60% fallback rate measures the fallback. **S29's CODE landed 2026-09-01; this is still blocked**, because the retry is untested against a record and the arm that would exercise it is v1, the 12/20 one this row's own reasoning names. | judgment | codex | a belfry record whose adjudicator retried | pre-committed criterion, control, scorer and frozen recipe |
