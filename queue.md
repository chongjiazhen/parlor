...
| **S26** | **Solver-seat control read.** Measure SolverPolicy versus random only where its entitled VOTE evidence can differ; no model and no cabal gate reopening. | judgment | codex | S15 implementation exists - **met** | control recipe, result and fallback rate, scoped as policy evidence |
...
| **S35** | **Per-call usage instrumentation.** Thread the reply's `usage` field and prompt/reply byte sizes from `core/backends.py` onto `Decision`. Additive and not model-facing, so it re-baselines nothing. **Landmines:** the draft copy-pastes 15 lines into all four `games/*/player.py` - four games IS the evidence `core/` asks for, so it lands in `core/` once; and `Decision` is serialised, so every reader must tolerate a record written before the fields existed. | constrained | opencode | `lane/slotB` carries a draft, 60 commits behind | the fields written ONCE in `core/`, an old record still readable, and a record showing non-zero usage |
