@echo off
setlocal
rem The waker-deck campaign - SETUP_6_WAKER, six seats, gated on a burst probe.
rem
rem Usage:  eval\runs\changeling-waker.cmd [tag] [games] [seed] [model]
rem
rem   eval\runs\changeling-waker.cmd waker1 200 12000 qwen36-35b-a3b-iq3
rem
...
rem misfires.
rem
rem THE CRITERION IS docs\changeling-waker-criterion.md AND IT BINDS, NOT THIS
...
rem --seats. Editing it in place to deal six would silently re-baseline every
...
set "TAG=%~1"
if "%TAG%"=="" set "TAG=waker1"
set "GAMES=%~2"
if "%GAMES%"=="" set "GAMES=200"
set "SEED=%~3"
if "%SEED%"=="" set "SEED=12000"
set "MODEL=%~4"
...
echo [gate] probe passed - %GAMES% games on SETUP_6_WAKER, seed %SEED%, model %MODEL%.>>"%LOG%"
...
python -m eval.run_changeling --games %GAMES% --arm llm --seats 6 --backend local ^
...
python -m eval.run_changeling --games %GAMES% --arm random --seats 6 ^
