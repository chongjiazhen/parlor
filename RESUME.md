...
| **S11** | **The DURF session engine - the half S8 did not take, and the one that tests the product claim.** The instrument scores rulings in isolation; it has no seats, so it does not exercise gate #1 at all. This builds the smallest thing that does: one fixed dungeon, 3-4 seats, a kernel implementing `docs/durf-rung.md`'s mechanics table and nothing else, and the adjudicator declaring its reveals as TYPED FACTS. Three constraints that are already decided and must not be re-litigated: fact-keyed entitlement is built inside `games/durf/` and does NOT move to `core/` (the `core/` invariant wants a second game asking, and one rung is not two); the entitlement snapshot is captured WITH the render, never recomputed at audit time, or a fact gone public by other means makes a real leak read clean; and `find_leaks` stays naive. Read §What is actually secret here first - DURF's secrets belong to the WORLD, so gate #1 collapses to one boundary and the peer-to-peer half cabal audits does not exist. | GPU only for a live arm; the audit half is free | the instrument exists (done 2026-08-28) | a render is audited against the facts the adjudicator declared, `find_leaks` unchanged, and gate #1 either holds or its failure is named and recorded |
...
**The queue reads: S11, then S8's untaken halves.** The adjudicator branch of S8 was taken 2026-08-28 and produced a dated VOID read rather than a closed slice; S11 is the engine half, and it ranks first because it is the only queued thing that exercises gate #1 on this rung - the instrument does not. The 6/7p package and the publish option are where they were. S2 and S5 landed, the powers
