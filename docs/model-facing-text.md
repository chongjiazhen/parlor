...
The doctrine for every string that lands in a player's context. It lives in
`docs/` rather than in the harness folder because it is engineering doctrine
about this repo, not tool configuration: a reader who has never run an agent
still needs to know that editing one of these strings is an experiment.
`.claude/rules/model-facing-text.md` is a six-line pointer at this file, and its
only job is to make an agent load this one when it opens
`core/backends.py`, `games/**/referee.py` or `games/**/player.py`.
