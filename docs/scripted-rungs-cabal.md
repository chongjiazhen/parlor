...
if-statements, and `docs/control-ladder.md` is why.

---

## 0. The part that generalises - MOVED to `docs/control-ladder.md`

Promotion trigger met 2026-09-02: changeling built its own ladder rung
(`games/changeling/heuristic.py`), so the game-free argument - why the ladder
keeps climbing on hand rules, the ceiling estimator protocol and the two
discipline constraints every rung inherits - now lives in
`docs/control-ladder.md`, verbatim. The three rungs below are cabal's and stay
here.
